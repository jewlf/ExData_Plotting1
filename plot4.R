...
## Plot 4 - Quad Plots for February 1 and 2, 2007
...
#par(bg="transparent")
# Set the global parameters, this time want a 2x2 grid of plots
par(mfcol=c(2,2))

# Upper-left plot, "Global Active Power"
plot(two_days_df$DateTime, two_days_df$Global_active_power, type="s", xlab=NA, ylab="Global Active Power")
...
plot(two_days_df$DateTime, two_days_df$Voltage, type="s", xlab="datetime", ylab="Voltage", lwd=0.1)

#Lower-right plot, "Global_reactive_power"
plot(two_days_df$DateTime, two_days_df$Global_reactive_power, type="s", xlab="datetime", ylab="Global_reactive_power", lwd=0.1)
